...
if ($action) {
    ...
    if ($action === 'list') {
        ...
        $where = ['1=1']; $params = [];
        if ($q) { $where[] = '(p.nama LIKE ? OR p.telepon LIKE ? OR p.alamat LIKE ?)'; $like="%$q%"; $params=[$like,$like,$like]; }
        if ($tipe) { $where[] = 'p.tipe=?'; $params[] = $tipe; }

        $whereStr = implode(' AND ', $where);

        // Total count
        $countStmt = $pdo->prepare("SELECT COUNT(DISTINCT p.id) FROM hl_pelanggan p WHERE $whereStr");
        ...
        $stmt = $pdo->prepare("SELECT p.*,
            ...
            FROM hl_pelanggan p
            LEFT JOIN hl_transaksi t ON t.pelanggan_id = p.id
            WHERE $whereStr
            GROUP BY p.id
            ORDER BY p.nama
    ...
    if ($action === 'save' && $_SERVER['REQUEST_METHOD']==='POST') {
        ...
        try {
            $pdo->exec("ALTER TABLE hl_pelanggan ADD COLUMN metode_bayar ENUM('langsung','bulanan') DEFAULT 'langsung'");
        } catch(Exception $e) {}

        $metodeBayar = $d['metode_bayar'] ?? 'langsung';
        if (!empty($d['id'])) {
            $pdo->prepare("UPDATE hl_pelanggan SET nama=?,telepon=?,alamat=?,tipe=?,catatan=?,metode_bayar=?,is_active=? WHERE id=?")
                ->execute([$d['nama'],$d['telepon'],$d['alamat'],$d['tipe'],$d['catatan'],$metodeBayar,$d['is_active'],$d['id']]);
        } else {
            // Cek duplikat telepon
            if (!empty($d['telepon'])) {
                $dup = $pdo->prepare("SELECT id FROM hl_pelanggan WHERE telepon=?");
                $dup->execute([$d['telepon']]);
                if ($dup->fetch()) { echo json_encode(['error'=>'Nomor HP sudah terdaftar']); exit; }
            }
            $pdo->prepare("INSERT INTO hl_pelanggan (nama,telepon,alamat,tipe,catatan,metode_bayar) VALUES (?,?,?,?,?,?)")
                ->execute([$d['nama'],$d['telepon'],$d['alamat'],$d['tipe'],$d['catatan'],$metodeBayar]);
        }
        logAudit(!empty($d['id'])?'update':'create','customer',(!empty($d['id'])?'Edit':'Tambah').' customer: '.($d['nama']??''));
        echo json_encode(['success'=>true]); exit;
    }
    if ($action === 'get_orders') {
        $id   = intval($_GET['id']);
        $rows = $pdo->prepare("SELECT t.no_order,t.tanggal,t.total,t.status_proses,t.status_bayar,
            GROUP_CONCAT(i.nama_layanan SEPARATOR ', ') as layanan
            FROM hl_transaksi t LEFT JOIN hl_transaksi_item i ON i.transaksi_id=t.id
            WHERE t.pelanggan_id=? GROUP BY t.id ORDER BY t.tanggal DESC LIMIT 20");
        $rows->execute([$id]);
        echo json_encode($rows->fetchAll()); exit;
    }
    if ($action === 'stats') {
        $total  = $pdo->query("SELECT COUNT(*) FROM hl_pelanggan WHERE is_active=1")->fetchColumn();
        $b2b    = $pdo->query("SELECT COUNT(*) FROM hl_pelanggan WHERE tipe='b2b' AND is_active=1")->fetchColumn();
        $baru   = $pdo->query("SELECT COUNT(*) FROM hl_pelanggan WHERE MONTH(created_at)=MONTH(CURDATE()) AND YEAR(created_at)=YEAR(CURDATE())")->fetchColumn();
