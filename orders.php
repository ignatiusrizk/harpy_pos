...
if ($action) {
    ...
    if ($action === 'list') {
        ...
        $sql = "SELECT t.*,
            (SELECT GROUP_CONCAT(nama_layanan SEPARATOR ', ') FROM hl_transaksi_item WHERE transaksi_id=t.id) as layanan_list
            FROM hl_transaksi t
            WHERE $whereStr
        ...
        $cnt = $pdo->prepare("SELECT COUNT(*) FROM hl_transaksi t WHERE $whereStr");
        $cnt->execute($params);
    ...
    if ($action === 'get') {
        ...
        $logs = $pdo->prepare("SELECT * FROM hl_proses_log WHERE transaksi_id=? ORDER BY created_at DESC LIMIT 10");
        $logs->execute([$id]);
        $t['logs'] = $logs->fetchAll();
    ...
    if ($action === 'update' && $_SERVER['REQUEST_METHOD']==='POST') {
        ...
        try {
            ...
            $old = $pdo->prepare("SELECT status_proses,status_bayar,catatan,dp FROM hl_transaksi WHERE id=?");
            $old->execute([$id]);
            $oldRow = $old->fetch();
            ...
            // Update header
            $stmt = $pdo->prepare("UPDATE hl_transaksi SET
            ...
            if (!empty($data['items'])) {
                $pdo->prepare("DELETE FROM hl_transaksi_item WHERE transaksi_id=?")->execute([$id]);
                ...
                $pdo->prepare("UPDATE hl_transaksi SET subtotal=? WHERE id=?")->execute([$subtotal, $id]);
            }
            ...
            // Insert semua logs
            $lstmt = $pdo->prepare("INSERT INTO hl_proses_log (transaksi_id,status_lama,status_baru,tipe,catatan,oleh) VALUES (?,?,?,?,?,?)");
            ...
            logAudit('payment','orders','Pembayaran order: '.($trx['no_order']??'').', Rp '.number_format($jumlah,0,',','.'), $trx['no_order']??null);
            $pdo->commit();
    ...
    if ($action === 'bayar' && $_SERVER['REQUEST_METHOD']==='POST') {
        ...
        $tipe     = $_POST['tipe_bayar'] ?? 'sebagian'; // sebagian / lunas
        $jumlah   = floatval($_POST['jumlah'] ?? 0);

        // Get current data
        $cur = $pdo->prepare("SELECT total, dp, sisa_bayar FROM hl_transaksi WHERE id=?");
        $cur->execute([$id]);
        $row = $cur->fetch();
        ...
        try {
            ...
            $pdo->prepare("INSERT INTO hl_proses_log (transaksi_id,status_lama,status_baru,tipe,catatan,oleh) VALUES (?,?,?,?,?,?)")
                ->execute([$id, $row['dp']>0?'dp':'belum_bayar', $new_status, 'bayar', $ket, $user['nama']]);

            // ── AUTO INSERT KAS MASUK ─────────────────────────
            // Buat tabel kas jika belum ada
            ...
            // Ambil no_order untuk referensi
            $trxData = $pdo->prepare("SELECT no_order, nama_pelanggan FROM hl_transaksi WHERE id=?");
            $trxData->execute([$id]);
            $trx = $trxData->fetch();
    ...
    if ($action === 'summary') {
        $pdo2 = getDB();
        $today  = $pdo2->query("SELECT COALESCE(SUM(total),0) FROM hl_transaksi WHERE DATE(tanggal)=CURDATE()")->fetchColumn();
        $month  = $pdo2->query("SELECT COALESCE(SUM(total),0) FROM hl_transaksi WHERE MONTH(tanggal)=MONTH(CURDATE()) AND YEAR(tanggal)=YEAR(CURDATE())")->fetchColumn();
        $statuses = ['masuk','cuci','kering','setrika','siap','diambil'];
        $sc = [];
        foreach ($statuses as $s) {
            $c = $pdo2->prepare("SELECT COUNT(*) FROM hl_transaksi WHERE status_proses=?");
            $c->execute([$s]);
            $sc[$s] = $c->fetchColumn();
        }
        echo json_encode(['today'=>$today,'month'=>$month,'statuses'=>$sc]);
        exit;
    }
    ...
    if ($action === 'wa_message') {
        $id   = intval($_GET['id']);
        $tipe = $_GET['tipe'] ?? 'reminder'; // reminder | siap | lunas_reminder
        $trx  = $pdo->prepare("SELECT * FROM hl_transaksi WHERE id=?");
        $trx->execute([$id]);
        $t = $trx->fetch();
        if (!$t) { echo json_encode(['error'=>'Not found']); exit; }
...
_Harpy Laundry | harpy.id_";
            ...
            // Default: status update reminder
...
<div class="modal-overlay" id="modalBayar" style="align-items:center;justify-content:center;padding:20px">
  <div class="modal" style="height:auto;max-height:90vh;width:480px">
    ...
    <div class="modal-body">
      ...
      <!-- Input nominal — muncul untuk keduanya -->
...
async function loadSummary() {
  ...
  document.getElementById('sAll').textContent    = Object.values(d.statuses).reduce((a,b)=>a+ +b,0);
...
function buildQuickNominal(sisa) {
  // Tombol quick nominal: sisa, lalu pembulatan ke atas
...
function selectTipe(tipe) {
  ...
  // Keduanya tampilkan nominal — lunas auto-isi sisa
...
async function submitBayar() {
  ...
  try {
    const r = await fetch('orders.php?action=bayar', { method:'POST', body: fd });
    const d = await r.json();
    if (d.success) {
      ...
      loadOrders();
      loadSummary();
      // Refresh modal detail kalau sedang buka
