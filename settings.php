...
if ($action) {
    ...
    if ($action === 'list_roles') {
        $rows = $pdo->query("SELECT r.*,
            (SELECT COUNT(*) FROM hl_users WHERE role_id=r.id AND is_active=1) as jumlah_user,
            (SELECT COUNT(*) FROM hl_role_permissions WHERE role_id=r.id) as jumlah_perm
            FROM hl_roles r ORDER BY r.is_system DESC, r.nama")->fetchAll();
    ...
    if ($action === 'get_role_perms') {
        ...
        $rows = $pdo->prepare("SELECT permission_id, filter_data FROM hl_role_permissions WHERE role_id=?");
        $rows->execute([$rid]);
        $map  = [];
        foreach ($rows->fetchAll() as $r) $map[$r['permission_id']] = $r['filter_data'];
        echo json_encode($map); exit;
    }

    // GET ALL PERMISSIONS (grouped by modul)
    if ($action === 'get_all_perms') {
        $rows = $pdo->query("SELECT * FROM hl_permissions ORDER BY modul, aksi")->fetchAll();
        $grouped = [];
    ...
    if ($action === 'save_role' && $_SERVER['REQUEST_METHOD']==='POST') {
        ...
        if (!empty($d['id'])) {
            // Tidak boleh rename system roles
            $isSystem = $pdo->prepare("SELECT is_system FROM hl_roles WHERE id=?");
            $isSystem->execute([$d['id']]);
            $sys = $isSystem->fetchColumn();
            if ($sys) {
                $pdo->prepare("UPDATE hl_roles SET deskripsi=?,is_active=? WHERE id=?")
                    ->execute([$d['deskripsi'],$d['is_active'],$d['id']]);
            } else {
                $pdo->prepare("UPDATE hl_roles SET nama=?,deskripsi=?,is_active=? WHERE id=?")
                    ->execute([$d['nama'],$d['deskripsi'],$d['is_active'],$d['id']]);
            }
        } else {
            $pdo->prepare("INSERT INTO hl_roles (nama,deskripsi,is_system) VALUES (?,?,0)")
                ->execute([$d['nama'],$d['deskripsi']]);
        }
        logAudit(!empty($d['id'])?'update':'create','settings','Role: '.($d['nama']??'edit'));
        echo json_encode(['success'=>true,'id'=>$pdo->lastInsertId()||$d['id']]); exit;
    }
    ...
    if ($action === 'save_perms' && $_SERVER['REQUEST_METHOD']==='POST') {
        ...
        $pdo->prepare("DELETE FROM hl_role_permissions WHERE role_id=?")->execute([$roleId]);
        $stmt = $pdo->prepare("INSERT INTO hl_role_permissions (role_id,permission_id,filter_data) VALUES (?,?,?)");
        foreach ($perms as $permId => $filter) {
            if ($filter) $stmt->execute([$roleId, $permId, $filter]);
        }
    ...
    if ($action === 'duplicate_role' && $_SERVER['REQUEST_METHOD']==='POST') {
        $d      = json_decode(file_get_contents('php://input'), true);
        $source = $pdo->prepare("SELECT * FROM hl_roles WHERE id=?");
        $source->execute([$d['id']]);
        $src    = $source->fetch();
        if (!$src) { echo json_encode(['error'=>'Role tidak ditemukan']); exit; }

        $pdo->prepare("INSERT INTO hl_roles (nama,deskripsi,is_system) VALUES (?,?,0)")
            ->execute(['Salinan ' . $src['nama'], 'Duplikat dari ' . $src['nama']]);
        $newId = $pdo->lastInsertId();

        // Copy permissions
        $perms = $pdo->prepare("SELECT permission_id,filter_data FROM hl_role_permissions WHERE role_id=?");
        $perms->execute([$d['id']]);
        $stmt  = $pdo->prepare("INSERT INTO hl_role_permissions (role_id,permission_id,filter_data) VALUES (?,?,?)");
...
// Update permission cache user saat ini
...
.modul-icons{gaster:4:'🧾',orders:'📋',kas:'💰',laporan:'📊',customer:'👥',karyawan:'👤',promo:'🎟️',layanan:'🧺',absensi:'🕐',settings:'⚙️'}
...
async function selectRole(id) {
  ...
  // Load current perms for this role
  const r = await fetch('settings.php?action=get_role_perms&role_id='+id);
  currentRolePerms = await r.json(); // {perm_id: filter}
